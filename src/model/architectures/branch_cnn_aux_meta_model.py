...
class BranchCNNAuxMetaModel(nn.Module):
    def __init__(
        self,
        imu_dim=11,
        tof_dim=25,
        ...
        super(BranchCNNAuxMetaModel, self).__init__()
        ...
        self.meta_default_emb_dim = self.default_emb_dim
        self.meta_blocks = nn.Sequential(
            nn.Linear(self.meta_dim, self.meta_default_emb_dim, bias=False),
            nn.BatchNorm1d(self.meta_default_emb_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(0.3),
            nn.Linear(
                self.meta_default_emb_dim, self.meta_default_emb_dim, bias=False
            ),
            nn.BatchNorm1d(self.meta_default_emb_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(0.3),
        )
        feature_with_meta_dim = (
            imu_out_filters
            + tof_out_filters
            + thm_out_filters
            + self.meta_default_emb_dim
        )
        # self.feature_meta_blocks = nn.Sequential(
        #     nn.Linear(feature_with_meta_dim, self.default_emb_dim, bias=False),
        #     nn.BatchNorm1d(self.default_emb_dim),
        #     nn.ReLU(inplace=True),
        #     nn.Dropout(0.3),
        #     nn.Linear(self.default_emb_dim, self.default_emb_dim * 2, bias=False),
        #     nn.BatchNorm1d(self.default_emb_dim * 2),
        #     nn.ReLU(inplace=True),
        #     nn.Dropout(0.3),
        # )
        # class_in_filters = self.default_emb_dim * 2
        class_in_filters = feature_with_meta_dim
    ...
    def forward(
        ...
        meta = self.meta_blocks(input_features["meta_features"])

        x = torch.cat([imu, tof, thm, meta], dim=-1)
        # x = self.feature_meta_blocks(x)
...
if __name__ == "__main__":
    ...
    meta_dim = 7  # Example meta dimension
    ...
    meta_input = torch.randn(batch_size, meta_dim)  # Example meta features
    dummy_input_simple = {
    ...
    output_simple = model(dummy_input_simple)
