...
class BranchCNNHalfAuxSplitHeadModel(nn.Module):
    def __init__(
        self,
        imu_dim=11,
        tof_dim=25,
        ...
        super(BranchCNNHalfAuxSplitHeadModel, self).__init__()
        ...
        print(
            f"Initializing Branch CNN Half SplitHead Aux Model with"
        ...
        self.target_gesture_classifier_head = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(0.3),
            nn.Linear(class_in_filters, 8),  # 8 target gestures
        )
        self.non_target_classifier_head = nn.Sequential(
            nn.Linear(class_in_filters, class_in_filters // 2, bias=False),
            nn.BatchNorm1d(class_in_filters // 2),
            nn.ReLU(inplace=True),
            nn.Dropout(0.3),
            nn.Linear(class_in_filters // 2, 10),
        )
    ...
    def forward(
        ...
        target_gesture_logits = self.target_gesture_classifier_head(x)
        non_target_logits = self.non_target_classifier_head(x)
        logits = torch.cat([target_gesture_logits, non_target_logits], dim=-1)
...
if __name__ == "__main__":
    ...
    simple_model = BranchCNNHalfAuxSplitHeadModel(imu_dim, tof_dim, thm_dim, n_classes)
