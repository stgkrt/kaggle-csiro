...
class BranchTransCNNAuxModel(nn.Module):
    def __init__(
        self,
        imu_dim=11,
        tof_dim=25,
        ...
        super(BranchTransCNNAuxModel, self).__init__()
        ...
        print(
            f"Initializing Branch Trans CNNAuxModel with"
    ...
    def forward(
        ...
        imu = input_features["imu_features"]
        imu = imu.permute(0, 2, 1)
        imu = self.input_conv(imu)
        imu = imu.permute(0, 2, 1)
        imu = self.transformer_block(imu)
        imu = self._get_signal_features(imu, self.imu_blocks)
...
if __name__ == "__main__":
    ...
    simple_model = BranchTransCNNAuxModel(imu_dim, tof_dim, thm_dim, n_classes)
