...
class ManyBranchCNNModel(nn.Module):
    def __init__(
        self,
        imu_dim=20,
        tof_dim=25,
        ...
        super(ManyBranchCNNModel, self).__init__()
        ...
        print(
            f"Initializing SimpleCNNModel with"
        ...
        self.imu_blocks.append(
            ResidualSECNNBlock(20, self.default_emb_dim, 3, drop=0.1)
        )
        ...
        self.other_imu_blocks.append(
            ResidualSECNNBlock(9, self.default_emb_dim, 3, drop=0.1)
        )
        other_imu_out_filters = self.default_emb_dim
        for _ in range(self.layer_num):
        ...
        self.tof_blocks.append(
            ResidualSECNNBlock(self.tof_dim, self.default_emb_dim, 3, drop=0.1)
        )
        tof_in_filters = self.default_emb_dim
        tof_out_filters = self.default_emb_dim
        ...
        self.thm_blocks.append(
            ResidualSECNNBlock(self.thm_dim, self.default_emb_dim, 3, drop=0.1)
        )
        thm_in_filters = self.default_emb_dim
        thm_out_filters = self.default_emb_dim
        for i in range(self.layer_num):
            if i % 2 == 0:
    ...
    def forward(
        ...
        basic_imu = input_features["imu_features"][:, :, :20]
        other_imu = input_features["imu_features"][:, :, 20:]
        imu = self._get_signal_features(basic_imu, self.imu_blocks)
        ...
        x = torch.cat([imu, other_imu, tof, thm], dim=-1)
