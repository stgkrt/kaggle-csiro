...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == "fit":
            imu_features = self._get_random_cut_features(imu_features)
            thm_features = self._get_random_cut_features(thm_features)
            tof_features = self._get_random_cut_features(tof_features)
        elif self.phase == "valid":
            imu_features = self._get_post_cut_features(imu_features)
            thm_features = self._get_post_cut_features(thm_features)
            tof_features = self._get_post_cut_features(tof_features)
        else:
...
if __name__ == "__main__":
    ...
    df_path = "/kaggle/working/processed_rotations/processed_with_rots_df.csv"
