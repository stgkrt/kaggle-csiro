...
from src.data.mixup_data import CutMixup, Mixup


class AugmentedAuxMixDataset(Dataset):
    ...
    def __init__(
        ...
        self.mixup_switch_epoch = mixup_switch_epoch
        ...
        input_features_dict = {
            "imu_features": self.imu_features_list,
            "thm_features": self.thm_features_list,
            "tof_features": self.tof_features_list,
        }
        labels_dict = {
            "label": self.labels,
            "orientation": self.orient_labels,
            "behavior": self.behavior_labels,
        }
        self.mixup = Mixup(X=input_features_dict, y=labels_dict)
        self.cutmixup = CutMixup(X=input_features_dict, y=labels_dict)
    ...
    def _get_random_cut_features(self, features):
        ...
        # padded_features = -np.ones((self.pad_len, features.shape[1]))
    ...
    def set_epoch(self, epoch):
        if epoch == self.mixup_switch_epoch:
            self.mixup_prob = 0.0
    ...
    def __getitem__(self, idx):
        ...
        orientation = self.orient_labels[idx]
        behavior = self.behavior_labels[idx]
        ...
        if self.phase == "fit":
            if np.random.rand() < self.mixup_prob:
                imu_features, label = self.mixup(imu_features, label)
                thm_features = self.mixup.same_idx_mix_input(
                    thm_features, "thm_features", "input"
                )
                tof_features = self.mixup.same_idx_mix_input(
                    tof_features, "tof_features", "input"
                )
                orientation = self.mixup.same_idx_mix_input(
                    orientation, "orientation", "label"
                )
                behavior = self.mixup.same_idx_mix_input(behavior, "behavior", "label")
            # if np.random.rand() < self.mixup_prob:
            #     imu_features, label = self.cutmixup(imu_features, label)
            #     thm_features = self.cutmixup.same_idx_mix_input(
            #         thm_features, "thm_features", "input"
            #     )
            #     tof_features = self.cutmixup.same_idx_mix_input(
            #         tof_features, "tof_features", "input"
            #     )
            #     orientation = self.cutmixup.same_idx_mix_input(
            #         orientation, "orientation", "label"
            #     )
            #     behavior = self.cutmixup.same_idx_mix_input(
...
if __name__ == "__main__":
    ...
    dataset = AugmentedAuxMixDataset(
        df,
        imu_cols=imu_cols,
        thm_cols=thm_cols,
        tof_cols=tof_cols,
