...
class RandomCutDataset(Dataset):
    def __init__(
        ...
        imu_cols = [
            col
            for col in colnames
            if col.startswith(("acc_", "rot_")) and not col.endswith("rolling_mean_5")
        ]
        # imu_cols = [col for col in colnames if col.startswith("acc_")]
        imu_cols = [col for col in imu_cols if not col.startswith(("rot_matrix_"))]
    ...
    def __getitem__(self, idx):
        ...
        padded_features = np.zeros((self.pad_len, len(self.features_cols)))
        ...
        if self.phase == "fit":
            ...
            random_signal_start_idx = np.random.randint(
                0, self.pad_len - cut_length + 1
            )
            padded_features[
        ...
        inputs = {"features": torch.Tensor(padded_features)}
        if self.phase == "fit" or self.phase == "valid":
            labels = {
